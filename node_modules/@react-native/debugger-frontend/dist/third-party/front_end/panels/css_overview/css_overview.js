...
<<<<<<< HEAD
        <h2>${z(W.textColorsS,{PH1:v.length})}</h2>
        <ul>
...
<<<<<<< HEAD
    </div>`,this.#p.element.appendChild(this.#y.element())}#$(e){const{payload:t}=e.data;let i="",o="";switch(t.type){case"contrast":{const{section:e,key:s}=t;i=`${e}-${s}`,o=z(W.contrastIssues);break}case"color":{const{section:e,color:s}=t;i=`${e}-${s}`,o=`${s.toUpperCase()} (${e})`;break}case"unused-declarations":{const{declaration:e}=t;i=`${e}`,o=`${e}`;break}case"media-queries":{const{text:e}=t;i=`${e}`,o=`${e}`;break}case"font-info":{const{name:e}=t;i=`${e}`,o=`${e}`;break}}let s=this.#C.get(i);if(!s){if(!this.#w||!this.#f)throw new Error("Unable to initialize CSS overview, missing models");s=new j(this.#n,this.#w,this.#f,this.#b),s.populateNodes(t.nodes),this.#C.set(i,s)}this.#m.appendTab(i,o,s,t.type)}#E(e){const t=Array.from(e.entries());return n.Fragment.Fragment.build`
=======
...
<<<<<<< HEAD
  </div>`}#L(e,t,i,o=""){const s=Array.from(e.entries()).sort(((e,t)=>{const i=e[1];return t[1].length-i.length})),r=s.reduce(((e,t)=>e+t[1].length),0);return n.Fragment.Fragment.build`<ul aria-label="${t}">
...
<<<<<<< HEAD
      `,u=h.$("container");return i?(u.append(n.Fragment.Fragment.build`<span>${z(W.apca)}</span>`.element()),this.data.thresholdsViolated.apca?u.appendChild(q()):u.appendChild(G())):(u.append(n.Fragment.Fragment.build`<span>${z(W.aa)}</span>`.element()),this.data.thresholdsViolated.aa?u.appendChild(q()):u.appendChild(G()),u.append(n.Fragment.Fragment.build`<span>${z(W.aaa)}</span>`.element()),this.data.thresholdsViolated.aaa?u.appendChild(q()):u.appendChild(G())),e.appendChild(h.element()),e}return super.createCell(t)}#z(e,i,o,s){const n=e.styleSheetHeaderForId(o);if(!n)return;const r=n.lineNumberInSource(s.startLine),a=n.columnNumberInSource(s.startLine,s.startColumn),l=new t.CSSModel.CSSLocation(n,r,a);return i.linkifyCSSLocation(l)}}function q(){const e=new l.Icon.Icon;return e.data={iconName:"clear",color:"var(--icon-error)",width:"14px",height:"14px"},e}function G(){const e=new l.Icon.Icon;return e.data={iconName:"checkmark",color:"var(--icon-checkmark-green)",width:"14px",height:"14px"},e}var Q=Object.freeze({__proto__:null,CSSOverviewCompletedView:B,DetailsView:U,ElementDetailsView:j,ElementNode:H}),K={cssText:`.css-overview-panel{overflow:hidden}devtools-css-overview-start-view{overflow:auto}\n/*# sourceURL=${import.meta.resolve("./cssOverview.css")} */\n`};class J extends n.Panel.Panel{#n;#_;#B;#U;#j;#H;#q;#G;#Q;#K;#J;#X;#Y;#Z;#ee;constructor(e){super("css-overview"),this.registerRequiredCSS(K),this.element.classList.add("css-overview-panel"),this.#n=e,this.#_=new v.CSSOverviewStartView.CSSOverviewStartView,this.#_.addEventListener("overviewstartrequested",(()=>this.#n.dispatchEventToListeners("RequestOverviewStart"))),this.#B=new T(this.#n),this.#U=new B(this.#n),t.TargetManager.TargetManager.instance().observeTargets(this),this.#n.addEventListener("RequestOverviewStart",(e=>{u.userMetrics.actionTaken(u.UserMetrics.Action.CaptureCssOverviewClicked),this.#te()}),this),this.#n.addEventListener("OverviewCompleted",this.#ie,this),this.#n.addEventListener("Reset",this.#d,this),this.#n.addEventListener("RequestNodeHighlight",this.#oe,this),this.#d()}targetAdded(e){if(e!==t.TargetManager.TargetManager.instance().primaryPageTarget())return;this.#U.initializeModels(e);const i=e.model(S);this.#j=i}targetRemoved(){}#se(){if(!this.#j)throw new Error("Did not retrieve model information yet.");return this.#j}#d(){this.#H=new Map,this.#q=new Map,this.#G=new Map,this.#Q=new Map,this.#K=new Map,this.#J=new Map,this.#X=new Map,this.#Y=0,this.#Z={styleRules:0,inlineStyles:0,externalSheets:0,stats:{type:0,class:0,id:0,universal:0,attribute:0,nonSimple:0}},this.#ee=new Map,this.#ne()}#oe(e){this.#se().highlightNode(e.data)}#ne(){this.#B.hideWidget(),this.#U.hideWidget(),this.contentElement.append(this.#_),this.#_.show()}#re(){this.#_.hide(),this.#U.hideWidget(),this.#B.show(this.contentElement)}#ae(){this.#_.hide(),this.#B.hideWidget(),this.#U.show(this.contentElement),this.#U.setOverviewData({backgroundColors:this.#H,textColors:this.#q,textColorContrastIssues:this.#ee,fillColors:this.#G,borderColors:this.#Q,globalStyleStats:this.#Z,fontInfo:this.#K,elementCount:this.#Y,mediaQueries:this.#J,unusedDeclarations:this.#X})}async#te(){this.#re();const e=this.#se(),[t,{elementCount:i,backgroundColors:o,textColors:s,textColorContrastIssues:n,fillColors:r,borderColors:a,fontInfo:l,unusedDeclarations:d},c]=await Promise.all([e.getGlobalStylesheetStats(),e.getNodeStyleStats(),e.getMediaQueries()]);i&&(this.#Y=i),t&&(this.#Z=t),c&&(this.#J=c),o&&(this.#H=o),s&&(this.#q=s),n&&(this.#ee=n),r&&(this.#G=r),a&&(this.#Q=a),l&&(this.#K=l),d&&(this.#X=d),this.#n.dispatchEventToListeners("OverviewCompleted")}#ie(){this.#ae()}}var X=Object.freeze({__proto__:null,CSSOverviewPanel:J});export{Q as CSSOverviewCompletedView,m as CSSOverviewController,y as CSSOverviewModel,X as CSSOverviewPanel,M as CSSOverviewProcessingView,N as CSSOverviewSidebarPanel,C as CSSOverviewUnusedDeclarations};
=======
